...
public class Lexicon {
	...
	public String checkLexicon(String tsLexiconName) {
		...
		if (lsReturn != null && !lsReturn.isEmpty()) {
			JSONArray loRet = new JSONArray(lsReturn);

			for (int lniCpt = 0; lniCpt < loRet.length(); lniCpt++) {
				JSONObject loLex = loRet.getJSONObject(lniCpt);
				if (loLex.getString("title").toLowerCase().equals(tsLexiconName.toLowerCase()))
					return loLex.getString("id");
			}			
		}
